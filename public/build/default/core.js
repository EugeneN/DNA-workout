/* Cafe 624d127c-ec3e-47f6-ae67-74373403e1f5 Mon Dec 31 2012 22:35:18 GMT+0200 (EET) */
...
(function() {
  ...
  if (!this.require) {
    ...
    partial = function(fn) {
      ...
      partial_args.shift();
      return function() {
        var a, arg, i, new_args, _len, _ref;
        _ref = [[], 0], new_args = _ref[0], arg = _ref[1];
        for (i = 0, _len = partial_args.length; i < _len; i++) {
          a = partial_args[i];
    ...
    require = function(name, root, ns) {
      var fn, module, path;
      if (name === 'bootstrapper') return window.bootstrapper;
...
require.define('', {
/*ZB:  /jquery-1.8.1 */
'jquery-1.8.1': function(exports, require, module) {(function() {
/*!
...
require.define('jquery-dom-parser', {
/*ZB:  jquery-dom-parser/JQueryDom */
'JQueryDom': function(exports, require, module) {(function() {
(function() {
  var IDom, jqidom, say;
  var __slice = Array.prototype.slice;
  ...
  jqidom = function(node) {
    ...
    return {
      ...
      get_by_attr: function(attr) {
        return (jnode.find(attr)).toArray();
      },
...
'index': function(exports, require, module) {(function() {
(function() {
  var CELLS, DA_EXTEND, DA_SUBSCRIBE, DEBUG, DNA_DATATYPES, THIS, compose2, cont_m, cont_t, default_handlers_cont, dispatch_handler, dispatch_handler_fn, dispatch_impl, domonad, find_cell, get_arity, get_cell, get_create_cell, get_create_cell_by_id, get_default_protocols, get_method_ns, get_protocol, interpose_handlers_with_events, is_array, is_async, is_data, is_handler, is_null, lift, lift_async, lift_sync, make_extended_node, make_monadized_handler, make_subscribed_node, maybe_m, parse_ast_handler_node, parse_genome, partial, register_protocol_impl, save_cell, say, synthesize_cell, synthesize_node, _ref, _ref1,
    __slice = [].slice,
  ...
  _ref = require('libprotocol'), register_protocol_impl = _ref.register_protocol_impl, dispatch_impl = _ref.dispatch_impl, get_protocol = _ref.get_protocol, get_default_protocols = _ref.get_default_protocols, is_async = _ref.is_async, get_arity = _ref.get_arity;

  _ref1 = require('libmonad'), cont_t = _ref1.cont_t, cont_m = _ref1.cont_m, maybe_m = _ref1.maybe_m, domonad = _ref1.domonad, is_null = _ref1.is_null, lift_sync = _ref1.lift_sync, lift_async = _ref1.lift_async;
  ...
  default_handlers_cont = function() {
    var args;
    args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
    return say("DNA monadic sequence finished with results:", args);
  };

  is_data = function(method) {
    var _ref2;
    return _ref2 = method.type, __indexOf.call(DNA_DATATYPES, _ref2) >= 0;
  };

  is_handler = function(method) {
    return !(is_data(method));
  };

  lift = function(h) {
    if (h.async) {
      return lift_async(h.arity, h);
    } else {
      return lift_sync(h.arity, h);
    }
  };
  ...
  synthesize_cell = function(node, protocols, dom_parser) {
    var all_the_protocols, proto_cell;
    if (!node.id) {
      node.id = (dom_parser.get_id(node)) || Math.uuid();
    }
    proto_cell = {
      id: node.id,
      node: node,
  ...
  get_create_cell = function(id, node, dom_parser) {
    var cell;
    if (cell = get_cell(id)) {
      return cell;
    } else {
  ...
  get_create_cell_by_id = function(id, dom_parser) {
    var node;
    if (node = dom_parser.get_by_id(id)) {
      return get_create_cell(id, node, dom_parser);
    } else {
      return null;
    }
  };

  dispatch_handler_fn = function(ns, method, cell) {
    switch (method.type) {
      case 'string':
        return {
          impl: function() {
  ...
  parse_ast_handler_node = function(handler, current_cell, dom_parser) {
    var cell, cell_id, handler_arity, handler_fn, handler_is_async, handler_ns, method, ns, real_handler, scope, _ref2;
    _ref2 = is_array(handler) ? handler[0] : handler, ns = _ref2.ns, method = _ref2.method, scope = _ref2.scope;
    cell_id = (scope != null ? scope.name : void 0) || THIS;
    cell = find_cell(cell_id, current_cell, dom_parser);
    handler_ns = is_handler(method) ? (ns != null ? ns.name : void 0) || (get_method_ns(method.name, cell)) : ns != null ? ns.name : void 0;
    handler_is_async = is_handler(method) ? is_async(handler_ns, method.name) : null;
    handler_arity = is_handler(method) ? get_arity(handler_ns, method.name) : null;
    if (!cell) {
    ...
    real_handler = is_array(handler) ? partial.apply(null, [handler_fn].concat(__slice.call(handler.slice(1).map(function(i) {
      return i.method.value;
    })))) : handler_fn;
    real_handler.async = handler_is_async ? true : false;
  ...
  make_extended_node = function(dom_parser, node) {
    var protocols;
    protocols = ((dom_parser.getData(DA_EXTEND, node)).split(" ")).filter(function(i) {
      return !!i;
    });
    say("Protocols found for", node, ":", protocols);
    return save_cell(synthesize_cell(node, protocols, dom_parser));
  };

  make_monadized_handler = function(dom_parser, cell, handlr) {
    var ast_parser, handlers_ast_list, lifted_handlers_chain, wrapper_monad;
    handlers_ast_list = is_array(handlr) ? handlr : [handlr];
    ast_parser = function(h) {
      return (parse_ast_handler_node(h, cell, dom_parser)).impl;
    };
    lifted_handlers_chain = handlers_ast_list.map(compose2(lift, ast_parser));
    wrapper_monad = cont_t(maybe_m({
      is_error: is_null
    }));
    return function(init_val) {
      say("Starting DNA monadic sequence with arguments:", init_val);
      return (domonad(wrapper_monad, lifted_handlers_chain, init_val))(default_handlers_cont);
    };
  };

  interpose_handlers_with_events = function(dom_parser, cell, handlers, evs_args) {
    var args, event, ns, raw_args, scope, _ref2, _ref3;
    _ref2 = is_array(evs_args) ? evs_args : [evs_args, []], (_ref3 = _ref2[0], ns = _ref3.ns, event = _ref3.event, scope = _ref3.scope), raw_args = 2 <= _ref2.length ? __slice.call(_ref2, 1) : [];
    args = (raw_args.filter(function(a) {
      var _ref4;
      return _ref4 = a.event.type, __indexOf.call(DNA_DATATYPES, _ref4) >= 0;
    })).map(function(a) {
      return a.event.value;
    });
    return handlers.map(function(handlr) {
      var _ref4;
      return (_ref4 = dispatch_handler(ns != null ? ns.name : void 0, event.name, find_cell((scope != null ? scope.name : void 0) || THIS, cell, dom_parser))).impl.apply(_ref4, args.concat([handlr]));
    });
  };

  make_subscribed_node = function(dom_parser, node) {
    ...
    return dna_sequences.map(function(dna_seq) {
      return dna_seq.events.map(partial(interpose_handlers_with_events, dom_parser, cell, dna_seq.handlers.map(partial(make_monadized_handler, dom_parser, cell))));
    });
  };

  synthesize_node = function(dom_parser) {
    var START_TIME, extended_nodes, root_node, subscribed_nodes;
    START_TIME = new Date;
    ...
    return say("Cells synthesis completed in " + (new Date - START_TIME) + "ms.");
  };

  module.exports = {
    start_synthesis: function(_arg) {
      var root_idom, root_node;
      root_node = (_arg != null ? _arg : {}).root_node;
...
require.define('Todo', {
/*ZB:  Todo/KeyPress */
'KeyPress': function(exports, require, module) {(function() {
(function() {
...
/*ZB:  Todo/TodoList */
'TodoList': function(exports, require, module) {(function() {
(function() {
  var ITodoList, List;
...
/*ZB:  Todo/views/list_item */
'views/list_item': function(exports, require, module) {(function() {
