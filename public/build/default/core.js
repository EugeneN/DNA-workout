/* Cafe c0e83bc7-a05d-451b-a5d4-55be036c1324 Sat Dec 29 2012 15:46:05 GMT+0200 (EET) */
...
'index': function(exports, require, module) {(function() {
(function() {
  var CELLS, DA_EXTEND, DA_SUBSCRIBE, DEBUG, DNA_DATATYPES, THIS, cont, cont_m, cont_t, dispatch_handler, dispatch_impl, domonad, find_cell, get_arity, get_cell, get_create_cell, get_create_cell_by_id, get_default_protocols, get_method_ns, get_protocol, is_async, is_data, is_handler, is_null, lab, lift_async, lift_sync, maybe_m, parse_ast_handler_node, parse_genome, register_protocol_impl, save_cell, say, synthesize_cell, _ref, _ref2;
  ...
  _ref2 = require('libmonad'), cont_t = _ref2.cont_t, cont_m = _ref2.cont_m, maybe_m = _ref2.maybe_m, domonad = _ref2.domonad, is_null = _ref2.is_null, lift_sync = _ref2.lift_sync, lift_async = _ref2.lift_async;
  ...
  synthesize_cell = function(node, protocols, dom_parser) {
    ...
    if (!node.id) node.id = (dom_parser.get_id(node)) || Math.uuid();
    proto_cell = {
      id: node.id,
      node: node,
      receptors: {},
  ...
  parse_ast_handler_node = function(handler, current_cell, dom_parser) {
    var cell, cell_id, handler_arity, handler_fn, handler_is_async, handler_ns, method, ns, partial_args, partially_applied_handler, scope, _ref3;
    ...
    if (Array.isArray(handler)) {
      ...
      if (handler_is_async) partially_applied_handler.async = true;
      partially_applied_handler.arity = handler_arity;
      return {
        impl: partially_applied_handler
      };
    } else {
      if (handler_is_async) handler_fn.impl.async = true;
      handler_fn.impl.arity = handler_arity;
      return handler_fn;
    }
  };

  lab = function(dom_parser) {
    var START_TIME, cell, cell_matrices, dna_sequences, node, proto_cells, protocols, root_node, _i, _j, _len, _len2, _results;
    if (DEBUG) window.dom_parser = dom_parser;
    ...
    for (_i = 0, _len = cell_matrices.length; _i < _len; _i++) {
      node = cell_matrices[_i];
    ...
    _results = [];
    for (_j = 0, _len2 = proto_cells.length; _j < _len2; _j++) {
      node = proto_cells[_j];
      ...
      dna_sequences.map(function(dna_seq) {
        var handlers;
        handlers = dna_seq.handlers.map(function(handlr) {
          var ast_parser, cont_maybe_m, handlers_ast_list, handlers_chain, lifted_handlers_chain;
          ...
          lifted_handlers_chain = handlers_chain.map(function(h) {
            if (h.async) {
          ...
          cont_maybe_m = cont_t(maybe_m({
            is_error: is_null
          }));
          return function(init_val) {
            say("Starting SNA monadic sequence with arguments:", init_val);
            return (domonad(cont_maybe_m, lifted_handlers_chain, init_val))(cont);
          };
        });
        return dna_seq.events.map(function(events) {
          var args, event, ns, raw_args, scope, _ref3, _ref4;
          _ref3 = Array.isArray(events) ? events : [events, []], (_ref4 = _ref3[0], ns = _ref4.ns, event = _ref4.event, scope = _ref4.scope), raw_args = 2 <= _ref3.length ? __slice.call(_ref3, 1) : [];
          args = (raw_args.filter(function(a) {
            var _ref5;
            return (_ref5 = a.event.type) === 'number' || _ref5 === 'string' || _ref5 === 'vector';
          })).map(function(a) {
          ...
          return handlers.map(function(handlr) {
            ...
            event_scope_cell = find_cell((scope != null ? scope.name : void 0) || THIS, cell, dom_parser);
            h = dispatch_handler(ns != null ? ns.name : void 0, event.name, event_scope_cell);
            if (args.length > 0) {
              return h.impl(args.concat([handlr]));
            } else {
      ...
      _results.push(say("Cells synthesis completed in " + (new Date - START_TIME) + "ms."));
    }
    return _results;
  };
  ...
  module.exports = {
    start_synthesis: function(_arg) {
      var idom, root_idom, root_node, _ref3;
      _ref3 = _arg != null ? _arg : {}, idom = _ref3.idom, root_node = _ref3.root_node;
      root_idom = idom || (dispatch_impl('IDom', document.body));
      root_idom.document().addEventListener("DOMNodeInserted", function(event) {
        return lab(dispatch_impl('IDom', event.target));
      });
      return lab(root_idom);
    },
    dump_cells: function() {
